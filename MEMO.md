...
- [x] **Inactive live refresh and AI priority (Aug 13, 2026)**: Delayed each vision `positionSequence` ACK until a two-phase inactive SwiftUI presentation completes; if no main window can present, the vision service retains and replays the newest position every 400 ms without restarting analysis. Warm live queries now explicitly terminate the session's residual `fullscan-*` work before submitting the high-priority `qidao-*` query. Regressions cover immediate model update, presentation-before-ACK within one second, non-default RunLoop delivery, ordered full-scan termination, and a first fake-engine candidate within five seconds.
